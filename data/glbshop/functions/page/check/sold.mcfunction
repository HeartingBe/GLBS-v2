...
#备份分数 @s GLBScheck_sold 到 #GLBScheck GLBSinfo
# #GLBScheck GLBSinfo 代表要查找的编号
scoreboard players operation #GLBScheck GLBSinfo = @s GLBScheck_sold
...
scoreboard players operation @e[type=minecraft:chest_minecart,distance=..3,limit=1,sort=nearest] GLBSsold = #GLBScheck_num GLBSinfo

#判断 if_check:1b
   #未找到
   execute unless data storage mny:glbs if_check run tellraw @s [">> \u00a7c未能在你的历史出售商品记录中找到编号为 ",{"score":{"name":"#GLBScheck","objective":"GLBSinfo"},"color":"yellow"}," \u00a7c的商品"]
   #找到
      #周围没有商店矿车 生成
      ...
      #修改矿车为查看历史购买
      execute if data storage mny:glbs if_check run data modify entity @e[type=minecraft:chest_minecart,tag=GLBShop,limit=1,sort=nearest] Tags set value ["GLBShop","GLBS6"]
      #通知
...
scoreboard players reset @s GLBScheck_sold