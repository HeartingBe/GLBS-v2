...
#通知
tellraw @p[tag=GLBSop,distance=..5] [">> 成功向出售商店添加商品,价格 ",{"score":{"name":"#GLBStemp_price","objective":"GLBSinfo"},"color":"yellow"}," \u00a7f金钱"]

#写进命令存储 mny:glbs modify 暂存
data modify storage mny:glbs modify set from entity @s Items[{Slot:10b}]

#标记为出售商店物品
data merge storage mny:glbs {modify:{tag:{glbs:{shop:1b,sell_op:1b}}}}
...
#写入Lore
   ...
   loot spawn ~ ~ ~ loot glbshop:sell_op
...
#把命令存储 mny:glbs modify 后置插入 mny:glbs sell_op
data modify storage mny:glbs sell_op append from storage mny:glbs modify
