#判断周围是不是有多于1个的玩家
   ...
   #玩家太多
   execute if score #GLBS_too_many_players GLBSinfo matches 2.. run tellraw @a[distance=..5] [">> \u00a7c购买失败,你周围有其他玩家"]

   #通过
   execute if score #GLBS_too_many_players GLBSinfo matches 1 run function mny:glbs/sell_op/pre_true
